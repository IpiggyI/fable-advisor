...
The architect writes the spec, picks the lane (rate limiting touches concurrency — a good case for racing `grok-implementer` against the codex runner and picking the stronger diff), and applies tiered acceptance when the report comes back: verification evidence plus a diff stat by default, with scoped or delegated full review as the stakes rise. Only then does it report done.
