...
Claude Code lets every subagent run on a different model — and lets the session itself run on a different model than its subagents. This plugin exploits that with the **architect pattern**: your session runs on a **flagship-tier** model (e.g. Fable 5 or Opus 5), acting as a full-time architect. It owns requirements, decomposition, specs, and verification — and routes every implementation task to the cheapest adequate lane:
...
| Fallback | Opus (in-house Claude) | `implementer` agent | The grok agent and the codex runner are both unavailable or not installed — keeps the plugin working with no external CLI (same family as the architect, so no cross-vendor review) |
...
- **In-house fallback (no CLI required):** if neither the Grok nor the Codex CLI is installed, the `implementer` agent runs the work directly on Claude Opus so the plugin stays self-contained and usable out of the box. Opus matches the architect's flagship tier and unit price — the win is context isolation (implementation detail stays out of the architect's context), not a cheaper rate. The trade-off is no cross-vendor review — its output shares the architect's model family — so prefer a CLI lane whenever one is available.
