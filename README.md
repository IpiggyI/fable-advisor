...
| Fallback | Sonnet / Opus (in-house Claude) | `implementer` agent | The grok agent and the codex runner are both unavailable or not installed — keeps the plugin working with no external CLI (same family as the architect, so no cross-vendor review) |
...
- **In-house fallback (no CLI required):** if neither the Grok nor the Codex CLI is installed, the `implementer` agent runs the work directly on Claude (Sonnet by default, `model="opus"` for high-stakes) so the plugin stays self-contained and usable out of the box. The trade-off is no cross-vendor review — its output shares the architect's model family — so prefer a CLI lane whenever one is available.
