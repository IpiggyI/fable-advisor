# fable-advisor

本仓库的公共词汇表：钉死最易记混的术语，供编排与决策散文统一取词。只定义"是什么"，不承载实现细节。

## Language

### 护栏与门禁

**receipt gate**:
Stop hook，fail open；针对的是**主会话自己**——排了 spec 却不跑、或把非-complete receipt 当完成的威胁。
_Avoid_: 笼统称"护栏/guardrail"而不指明针对主会话；勿与历史上的 spawn 护栏混同（后者针对 wrapper 子代理，已随 wrapper 一并退役，见 ADR 0009）。

**lane family gate**:
Cursor 用户级 `preToolUse` 门：`Task` 派发 `fable-advisor` / `implementer` 时必须钉对家族模型。
_Avoid_: 与 receipt gate 混称；称作插件 hook 或本仓 project hook；把用户级 Task pin 规则当成另一套门禁。

### 实现车道

**lane**:
实现工作的四条可选车道（Routine / Cross-vendor / In-house / Handoff），按任务性质与可用性择一，而非四种同义叫法；其中 Handoff 是条件成员，仅在用户声明后进入候选。
_Avoid_: 把任意实现路径都叫"lane"而不区分四条；把车道名与具体模型型号混为一谈。

**Routine lane**:
spec 完全决定结果时的默认实现车道，经 grok runner 执行。
_Avoid_: 把"默认"理解成"唯一"或"可随意改道而不说明依据"。

**Cross-vendor lane**:
正确性或完备性关键、需要第二家独立实现时的车道，经 codex runner 执行。
_Avoid_: 与 Routine 互换使用；把"跨厂"理解成任意备用路径。

**In-house lane（仓内车道）**:
与架构师同族、仓内自含、无外部 CLI 依赖的实现车道；可按专长命中、上下文隔离、额度/时限声明主动路由，也承担两条 CLI lane 均不可用时的兜底。**同模派发**是它的一个拨盘：模型钉为会话模型而非默认别名槽位，只用于准则散文类交付物（插件自身的 skill / agent 文本），理由是与架构师判断的一致性。
_Avoid_: 把它当成无代价的默认选项而不披露三条固定代价（同族无跨厂评审、共享主会话额度、单价最贵）；用型号名（Opus/Sonnet）指称该车道；把同模派发当成第五条车道；按"够不够核心"而不是产物类别决定是否同模。

**Handoff lane（交接车道）**:
用户中介的实现车道：架构师把五部 spec 与操作指南写入 `.fable-advisor/handoff/<slug>.md`，由用户手动带到自己选定的 harness 执行，回来后架构师读 diff、亲自重跑 verification 验收；无机械门禁（不在 receipt gate 视野内）、fail open，默认不在候选集内，仅在用户显式声明后成为 stage 2 的条件成员。
_Avoid_: 未经用户声明将其纳入路由候选；因"免费"把小任务也交接出去（往返开销压倒收益）；把回传报告当验收证据（证据是 diff）。

### 会话工作模式

**架构师层**:
由会话模型身份决定的工作模式：会话模型属旗舰层系列时，写交付契约、路由车道、验收结果；只写协调件，不写实现。
_Avoid_: 用具体型号名指称该模式；把"架构师"当成任意会写代码的会话；改称组长 / 老师 / 管理者；把"写契约"理解成替车道写实现步骤。

**advisor-only**:
由会话模型身份决定的工作模式：会话模型不属于旗舰层系列时，自己实现，仅在承诺边界咨询顾问。
_Avoid_: 与架构师层混称；把"顾问"理解成另一种实现车道。

### 交付契约与产物

**交付契约（spec）**:
派给车道的五部任务书（Objective / Files / Interfaces / Constraints / Verification）：只规定做什么、授权范围、保留的接口与约束、怎样证明完成；Constraints 未规定的实现选择默认归车道。
_Avoid_: 写成施工说明或步骤教程；把"没写怎么做"当成契约不完整；把 Files 当成穷尽的改动清单而不是授权范围。

**契约缺口**:
交付契约里车道无法自行解决的空洞：预期行为不明、两条要求冲突、必须改动保留接口、无法判定什么算通过。车道上报，架构师修正契约。
_Avoid_: 与"未规定的实现选择"混称；用"use your judgment"回应真正的缺口。

**未规定的实现选择**:
契约边界内、Constraints 没有钉死的局部决定：内部函数划分、等价数据结构、测试组织、契约范围内的错误处理。车道自行决定，不上报、不等待。
_Avoid_: 当成契约缺口退回架构师；拿它当改动保留接口或扩大范围的理由。

**返工票**:
评审未过后回到原车道的派发：只写违反的要求、可复现的问题、期望行为、验证方式，不写修复方案。
_Avoid_: 架构师亲手修改后宣布通过；因结构偏好而非契约违反开返工票。

**交付物**:
随插件发布、被测试、或对外描述行为的产物。一律经车道修改，架构师不亲手改。
_Avoid_: 按改动大小而不是按产物类别判断能否亲手改；把"只是补一个测试 / 修个拼写"当作例外。

**协调件**:
架构师用来组织落地的记录：任务件（issue、spec）、决策记录（ADR、词表）、工作流状态文件、发布版本字段。架构师可直接写。
_Avoid_: 把协调件当成交付物走车道；借协调件之名修改交付物。

### 模型别名

**别名槽位**:
`opus` / `sonnet` / `haiku` / `fable` 这类模型别名是可被环境变量重定向的**指针**，不是模型身份。
_Avoid_: "跑在 Opus 槽位 = 跑的是 Opus"这类说法；把别名名当成已解析的具体型号。
