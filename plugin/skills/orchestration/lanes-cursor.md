...
- **Pin explicitly, every time.** Agent frontmatter `model:` is not honored for plugin-loaded agents in Cursor, and Task inherits the session model when `model` is omitted — an unpinned dispatch silently becomes whatever the session runs. The user-level lane family gate denies a `fable-advisor` dispatch whose `model` is missing or `inherit`; it does not check the family, and it does not cover `generalPurpose`, so an unpinned worker is your mistake to catch. Use the slug live in this turn's allowlist; a skill example may name a generation the allowlist lacks.
...
- **Economics unchanged.** Each vendor's models draw on their own quota pool in Cursor, so lane prices — and the whole cost discipline — apply as written.
