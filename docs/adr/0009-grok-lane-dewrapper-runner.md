...
复盘条件追加：出现 `idle_timeout` 回执且其 `max_idle_ms` 接近 600 秒 → 存在合法的长静默，调大 N 或改找别的活性信号；codex 车道积累够样本后单独校准 N；`timeout_sec` 无默认值导致真实的失控长跑（靠 Bash `timeout` 才终止）→ 重新给绝对上限设默认值。
