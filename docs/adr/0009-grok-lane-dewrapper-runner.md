...
## 追记（2026-09-09）— 静默截止取代总墙钟，准备窗口只管非续跑

上一条复盘条件触发：`max_idle_ms` 分布已足够。

**分布（19 份非空样本，cc-usage 2026-09-08 的 grok 串行批）：** min 33 秒、p50 66 秒、p90 121 秒、max 134 秒。这些跑的总时长 641–1725 秒，且各自按前言跑过 typecheck 与全量测试——上一条里「CLI 在长命令期间完全不吐事件」的分支不成立，验证阶段的最长静默段仍在同一量级。两次 `timeout` 回执被砍时的 `max_idle_ms` 是 76.6 秒与 43.4 秒，18 次 `complete` 里 15 次总时长超过 1080 秒：墙钟砍掉的是活车道，总时长与活性不相关。

**决策 1：杀死判据改为静默截止（两条 runner）。** 新 spec 字段 `idle_timeout_sec`，默认 600 秒（观测上界 134 秒的约 4.5 倍余量），语义是最后一个事件之后多久无新事件才杀；计时复用 `max_idle_ms` 的逐行观测点，每个事件重新武装。`timeout_sec` 语义收窄为可选的绝对上限且不再有默认值。新 `error_class: idle_timeout`，与 `timeout` 同一条收尾路径（杀进程组、跳过验证、保留 pending、非零退出）。receipt 增记 `idle_timeout_sec` 与 `timeout_sec` 的实际生效值，`receipt_version` 仍为 1。取舍：默认无绝对上限后，前台派发的实际天花板落到 harness 的 Bash `timeout`（上限 3600000 毫秒）。

**决策 2：30 秒准备窗口只在非续跑时武装。** 前提证伪——上一条把续跑失败归给「被 SIGKILL 的会话不可恢复」，不成立：全库 11 份带 `resumed_from` 的回执里，父跑被 SIGKILL（`timeout`）的 5 份有 3 份续跑 `complete`；被杀会话的磁盘数据完好（`68e7fa5c…` 目录 3.0 MB，写到被杀前一秒）。两份失败的续跑都停在 32.7 秒左右，正是 `PREPARATION_TIMEOUT_MS` 加开销——续跑要重放数 MB 会话历史，固定 30 秒本身就是第二个瞎墙钟。修订：续跑的首个事件由静默截止管；非续跑仍是 30 秒。

**决策 3：进程关闭后只要从未观测到事件就判 `preparation_stalled`，不看是否续跑。** 否则续跑 + CLI 静默退出 0 会落到验证分支，脏树上被判 `complete`——一次零产出的派发当成交付。

已知边界：19 份样本全部来自 grok 车道，codex 车道零样本，600 秒对它是同量级外推而非实测；30 秒准备窗口本身无测试覆盖（避免 30 秒级的真实等待），覆盖的是关闭后的归类。

复盘条件追加：任一车道出现 `idle_timeout` 回执且其 `max_idle_ms` 接近 600 秒 → 存在合法的长静默，调大 N 或改找别的活性信号（codex 车道样本为零，这条同样适用于它）；`timeout_sec` 无默认值导致真实的失控长跑（靠 Bash `timeout` 才终止）→ 重新给绝对上限设默认值。
