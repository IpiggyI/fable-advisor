...
当用户已声明 Handoff lane 时阅读本文（声明规则在 SKILL.md 的「用户路由 profile」）。其他每条 lane 都要求本会话能够调用生产者；Handoff lane 放弃这一点：用户把工作带到自选的 harness（一份边际成本 ≈ 0 的固定订阅）并把结果带回来。架构师产出的是一个文件，不是一个进程。
